...
import org.bukkit.craftbukkit.maps.CraftMapView;
...
public class ItemWorldMap extends ItemWorldMapBase {
	...
	public WorldMap a(ItemStack itemstack, World world) {
		...
		WorldMap worldmap = (WorldMap) world.a(WorldMap.class, "map_"
				+ itemstack.getData());
		...
		if (worldmap == null) {
			...
			// Craftbukkit start
			CraftMapManager.getSingleton().activate(new CraftMapView(worldmap));
			// Craftbukkit end
	...
	public void a(World world, Entity entity, WorldMap worldmap) {
		if (((WorldServer) world).dimension == worldmap.d) { // CraftBukkit
			...
			// CraftBukkit start
			if (CraftMapManager.getSingleton().MapUpdated(worldmap, entity,
					MapType.Base)) {
				return;
			}
			// CraftBukkit end
			...
			int i1 = MathHelper.floor(entity.locZ - (double) k) / i + short2
					/ 2;
			int j1 = 128 / i;
								...
								for (l4 = 0; l4 < i; ++l4) {
									for (j4 = 0; j4 < i; ++j4) {
										...
										if (k4 > 1) {
											...
											do {
												flag1 = true;
												j5 = chunk.getTypeId(l4 + k3,
														k4 - 1, j4 + l3);
												if (j5 == 0) {
													flag1 = false;
												} else if (k4 > 0
														&& j5 > 0
														&& Block.byId[j5].material.A == MaterialMapColor.b) {
													flag1 = false;
												}

												if (!flag1) {
													--k4;
													j5 = chunk.getTypeId(l4
															+ k3, k4 - 1, j4
															+ l3);
												}
											} while (!flag1);
	...
	public void a(ItemStack itemstack, World world, Entity entity, int i,
			boolean flag) {
		if (!world.isStatic) {
	...
	public Packet b(ItemStack itemstack, World world, EntityHuman entityhuman) {
		//Craftbukkit start
		String mapName = "map_" + itemstack.getData();
		MapView map = CraftMapManager.getSingleton().getMapView(mapName);
		if(map.isContextual())
			map.getCanvas(MapType.Base).setCurrentUser((CraftPlayer)((EntityPlayer)entityhuman).getBukkitEntity());
		//Craftbukkit end

		byte[] abyte = this.a(itemstack, world).a(itemstack, world, entityhuman); 
	
		
		return abyte == null ? null : new Packet131((short) Item.MAP.id,
				(short) itemstack.getData(), abyte);
	}
}
