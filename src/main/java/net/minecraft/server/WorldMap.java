...
public class WorldMap extends WorldMapBase {
	...
	public void a(EntityHuman entityhuman, ItemStack itemstack) {
		if (!this.j.containsKey(entityhuman)) {
			WorldMapHumanTracker worldmaphumantracker = new WorldMapHumanTracker(
					this, entityhuman);
			// Craftbukkit start
		...
		for (int i = 0; i < this.h.size(); ++i) {
			WorldMapHumanTracker worldmaphumantracker1 = (WorldMapHumanTracker) this.h
					.get(i);

			if (!worldmaphumantracker1.a.dead
					&& worldmaphumantracker1.a.inventory.c(itemstack)) {
				float f = (float) (worldmaphumantracker1.a.locX - (double) this.b)
						/ (float) (1 << this.e);
				float f1 = (float) (worldmaphumantracker1.a.locZ - (double) this.c)
						/ (float) (1 << this.e);
				byte b0 = 64;
				byte b1 = 64;

				if (f >= (float) (-b0) && f1 >= (float) (-b1)
						&& f <= (float) b0 && f1 <= (float) b1) {
					byte b2 = 0;
	...
	public byte[] a(ItemStack itemstack, World world, EntityHuman entityhuman) {
		WorldMapHumanTracker worldmaphumantracker = (WorldMapHumanTracker) this.j
				.get(entityhuman);
